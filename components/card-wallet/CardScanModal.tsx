...
import type { CardCrop, CardWalletItem } from "@/lib/cardWallet";
...
export function CardScanModal({
  ...
  onClose: () => void;
  onCrop: () => void;
  onSaveCrop: (crop: CardCrop) => Promise<void>;
}) {
  const [scanCrop, setScanCrop] = useState(crop);

  async function update(next: CardCrop) {
    setScanCrop(next);
    await onSaveCrop(next);
  }
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col overflow-hidden bg-white text-zinc-950 [overscroll-behavior:contain]">
      <div className="flex items-center justify-between px-4 pb-3 pt-[calc(0.9rem+env(safe-area-inset-top))]">
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">{card.codeType === "qr" ? "QR" : "Barcode"}</p>
      ...
      <div className="grid grid-cols-4 gap-2 border-t border-zinc-200 px-4 pb-[calc(0.9rem+env(safe-area-inset-bottom))] pt-3">
        ...
        <button className="rounded-full bg-zinc-100 px-3 py-2 text-sm" onClick={() => update({ ...scanCrop, rotate: ((scanCrop.rotate + 90) % 360) as CardCrop["rotate"] })}>旋转</button>
        <button className="rounded-full bg-zinc-900 px-3 py-2 text-sm text-white" onClick={onCrop}>裁剪</button>
      </div>
    </div>
  );
}
