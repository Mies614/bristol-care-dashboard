...
export function inferNoteType(input: { content?: string; imageUrl?: string; audioUrl?: string; videoUrl?: string }): LoveNote["noteType"] {
  const count = [input.content?.trim(), input.imageUrl, input.audioUrl, input.videoUrl].filter(Boolean).length;
...
export function hasNoteContent(input: { content?: string; imageUrl?: string; audioUrl?: string; videoUrl?: string }) {
  return Boolean(input.content?.trim() || input.imageUrl || input.audioUrl || input.videoUrl);
}
